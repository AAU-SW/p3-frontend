...
import { ChevronDown, ChevronUp, Bell, BellDot } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ auth }) => {
  ...
  const firstLetter = auth.user?.name ? auth.user.name.charAt(0).toUpperCase() : 'U';
  ...
  return (
    <header className="h-19 flex items-center justify-between px-6 border-b shadow-sm bg-white">
        ...
        <Avatar>
          ...
          <Button onClick={setIsOpen.bind(null, !isOpen)} variant="ghost" size="icon-sm">
            {isOpen ? <ChevronDown className='w-4 h-4'/> : <ChevronUp className='w-4 h-4'/>}
          </Button>
          <Button className="-ml-4" onClick={handleClick} variant="ghost" size="icon-sm">
            {notifications > 0 ? <BellDot className="w-4 h-4" /> : <Bell className="w-4 h-4" />}
          </Button>
          { seetNotifications &&
